...
POSE_RESNET.USE_FFT = [0, 0, 0]
POSE_RESNET.NLTYPE = 'fft'
POSE_RESNET.SPECTRUM_RELU = False
POSE_RESNET.FINAL_CONV_KERNEL = 1
POSE_RESNET.TARGET_TYPE = 'gaussian'
POSE_RESNET.HEATMAP_SIZE = [64, 64]  # width * height, ex: 24 * 32
POSE_RESNET.SIGMA = 2
POSE_RESNET.ENC_DS = [2, 2]
POSE_RESNET.ENC_NL = [1000, 1000, 1000]
...
def _update_dict_rec(d1, d2):
    for k, v in d2.items():
        if k in d1:
            if isinstance(v, dict):
                _update_dict_rec(d1[k], v)
            else:
                d1[k] = v
        else:
            raise ValueError("{}.{} not exist in config.py".format(k, v))
